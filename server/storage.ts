import { 
  ...
  FinancingApplication, InsertFinancingApplication,
  TestDrive, InsertTestDrive,
  TradeIn, InsertTradeIn,
} from "./schema";
...
export class MemStorage implements IStorage {
    ...
    if (filters) {
      vehicles = vehicles.filter(vehicle => {
        return Object.entries(filters).every(([key, value]) => {
          ...
          if (key === 'year' && typeof value === 'object') {
            const min = (value as any).min;
            const max = (value as any).max;
            if (min !== undefined && vehicle.year < min) return false;
            if (max !== undefined && vehicle.year > max) return false;
            return true;
          }
          
          // Special case for mileage range
          if (key === 'mileage' && typeof value === 'object') {
            const min = (value as any).min;
            const max = (value as any).max;
            if (min !== undefined && vehicle.mileage < min) return false;
            if (max !== undefined && vehicle.mileage > max) return false;
            return true;
          }
          
          // Special case for multiple fuel types
          if (key === 'fuelType' && Array.isArray(value)) {
            return value.includes(vehicle.fuelType);
          }
          
          // Special case for model (partial match)
          if (key === 'model' && typeof value === 'string') {
            return vehicle.model.toLowerCase().includes(value.toLowerCase());
          }
          
          // Special case for make (partial match)
          if (key === 'make' && typeof value === 'string') {
            return vehicle.make.toLowerCase().includes(value.toLowerCase());
          }
  ...
  async createVehicle(insertVehicle: InsertVehicle): Promise<Vehicle> {
    ...
    const vehicle: Vehicle = { 
      ...
      images: insertVehicle.images || null,
      isFeatured: insertVehicle.isFeatured || false,
      mpg: insertVehicle.mpg || null,
      range: insertVehicle.range || null,
      monthlyPayment: insertVehicle.monthlyPayment || null,
      reservedBy: insertVehicle.reservedBy || null,
      reservedUntil: insertVehicle.reservedUntil || null,
      soldTo: insertVehicle.soldTo || null,
      soldAt: insertVehicle.soldAt || null
    };
  ...
  async createFinancingApplication(insertApplication: InsertFinancingApplication): Promise<FinancingApplication> {
    ...
    const application: FinancingApplication = { 
      ...
      employmentStatus: insertApplication.employmentStatus || null,
      annualIncome: insertApplication.annualIncome || null,
      vehicleId: insertApplication.vehicleId || null
    };
  ...
  async createTestDrive(insertTestDrive: InsertTestDrive): Promise<TestDrive> {
    ...
    const testDrive: TestDrive = { 
      ...
      id, 
      createdAt,
      status: insertTestDrive.status || "scheduled",
      userId: insertTestDrive.userId || null
    };
  ...
  async createTradeIn(insertTradeIn: InsertTradeIn): Promise<TradeIn> {
    ...
    const tradeIn: TradeIn = { 
      ...
      createdAt, 
      estimatedValue,
      status: insertTradeIn.status || "pending",
      userId: insertTradeIn.userId || null,
      trim: insertTradeIn.trim || null
    };
